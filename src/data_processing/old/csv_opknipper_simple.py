...
class FixedDialect(csv.Dialect):
    delimiter = DEFAULT_DELIMITER
    doublequote = True
    escapechar = None
    lineterminator = '\n'
    quotechar = '"'
    quoting = csv.QUOTE_MINIMAL   # voorkomt 'need to escape' fout
    skipinitialspace = False

def csv_dict_reader(file_path: str):
    """
    Leest CSV met vaste encoding en delimiter, zonder sniffing.
    Geeft (fp, DictReader, header list, dialect, encoding).
    """
    fp = open(file_path, 'r', encoding=DEFAULT_ENCODING, newline='')
    base_reader = csv.reader(fp, dialect=FixedDialect)
    try:
        header = next(base_reader)
    except StopIteration:
        fp.close()
        return None, None, None, None, None
    reader = csv.DictReader(fp, fieldnames=header, dialect=FixedDialect)
    return fp, reader, header, FixedDialect, DEFAULT_ENCODING



def csv_dict_writer(file_path: str, fieldnames):
    """
    Schrijft CSV met vaste encoding en delimiter, zonder sniffing.
    """
    fp = open(file_path, 'w', encoding=DEFAULT_ENCODING, newline='')
    writer = csv.DictWriter(fp, fieldnames=fieldnames, dialect=FixedDialect)
    writer.writeheader()
